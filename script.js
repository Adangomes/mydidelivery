...
// --- 1. CARREGAMENTO E RENDERIZAÇÃO (CORRIGIDO PARA MOSTRAR TUDO) ---
...
function renderizarCardapio() {
    ...
    categorias.forEach((cat, idx) => {
        ...
        produtosGeral.filter(p => p.categoria === cat).forEach(p => {
            // REMOVIDO: os filtros que barravam pizzas e porções
...
function decidirFluxo(nome) {
    ...
    // Se for pizza, porção ou tiver marcado como abrir_modal no JSON
    if (p.categoria === 'pizza' || p.categoria === 'porcao' || p.tipo_escolha === 'abrir_modal') {
...
function abrirModalSelecao(nome) {
    ...
    if (itemMestreTemporario.categoria === 'pizza') {
        ...
        // Lógica para porções (600g = P, 1kg = G)
...
function montarListaSabores(n, tipo) {
    ...
    // Pega apenas os sabores que não são os "títulos" (aqueles com preço 0 no JSON)
    const opcoes = (tipo === 'pizza') 
        ? produtosGeral.filter(p => p.categoria === 'pizza' && p.price === 0) 
        : produtosGeral.filter(p => p.categoria === 'porcao' && p.price === 0);
...
function enviarWhatsApp() {
    ...
    if (typeof salvarPedidoFirebase === 'function') {
        salvarPedidoFirebase({ nome, rua, num, bairro, pag }).then(finalizarEVoltarInicio).catch(finalizarEVoltarInicio);
    } else {
